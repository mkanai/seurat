...
#' @export
#'
SCTransform_step2 <- function(
  object,
  layer = 'counts',
  cell.attr = NULL,
  reference.SCT.model = NULL,
  step1.assay.out = NULL,
  do.correct.umi = TRUE,
  ncells = 5000,
  residual.features = NULL,
  variable.features.n = 3000,
...
SCTransform_step1_ <- function(
  ...
  # from SCTransform.Seurat
  if (is.null(latent.data) & any(vars.to.regress %in% colnames(x = object[[]]))) {
    vars.to.regress.subset <- vars.to.regress[vars.to.regress %in% colnames(x = object[[]])]
    latent.data <- object[[vars.to.regress.subset]]
  }
  if (is.null(cell.attr)) {
    cell.attr <- slot(object = object, name = 'meta.data')[colnames(object[["RNA"]]),]
  }
  ...
  for (dataset.index in seq_along(along.with = layers)) {
    ...
    if (length(x = cells.grid) == 1){
      ...
      if (write.on.disk) {
        ...
        correct_counts <- BPCells::write_matrix_dir(mat = BPCells::convert_matrix_type(corrected_counts, "uint32_t"), tmpdir)
        return(list(SCT=list(counts=correct_counts)))
      }
      cell_attrs <- Reduce(rbind, cell_attrs)
      vst_out.reference$cell_attr <- cell_attrs[colnames(new.residuals),,drop=FALSE]
      SCTModel.list <- PrepVSTResults(vst.res = vst_out.reference, cell.names = all_cells)
